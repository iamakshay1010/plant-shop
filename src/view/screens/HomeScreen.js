...
export default function HomeScreen({ navigation }) {
      ...
      <View style={style.categoryContainer}>
        {categories.map((item, index) => (
          <TouchableOpacity
            key={index}
            activeOpacity={0.8}
            onPress={() => setCategoryIndex(index)}>{/*---setting index from here*/}
            <Text
              style={[
                style.categoryText,
  ...
  const Card = ({plant}) => {
    return (
      <TouchableOpacity
        activeOpacity={0.8}
        onPress={() => navigation.navigate('Details', plant)}>{/*-----move to next screen with data--------*/}
        <View style={style.card}>
          <View style={{alignItems: 'flex-end'}}>{/*-----to show icon itms on top right --------*/}
            <View
              style={{
                width: 30,
                height: 30,
